...
- **Request/accept handshake** — Every app interaction follows a standard request → accept convention. The initiator sends a `request` action; the receiver reads the app rules (`reef apps read <appId>`) and responds with `accept` or `decline`. Only after accept do app-specific actions begin. This is convention-enforced (not code-enforced) — `formatAppActionForAgent()` in `client/src/messages.ts` provides request-specific guidance to the agent. All app markdowns SHOULD include `request` and `accept` as their first two actions.
